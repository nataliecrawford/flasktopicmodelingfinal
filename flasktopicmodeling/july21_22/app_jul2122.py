...
def upload_file():
    if request.method == 'POST':
        ...
        returndata = topicmodelinga(f.filename)
...
def topicmodelinga(value):
    ...
    pofdocstotopics = "/Users/nataliecrawford/Downloads/Mallet-202108/bin/mallet train-topics --input /Users/nataliecrawford/Downloads/"+ strvalue + ".mallet \
                    --num-topics 10 --num-iterations 1000 --output-doc-topics /Users/nataliecrawford/Downloads/"+ strvalue +".xlsx"
    os.system(pofdocstotopics)

    exceldata = pd.read_table("/Users/nataliecrawford/Downloads/"+ strvalue+".xlsx")
    df = pd.DataFrame(exceldata)
    #adds columns to easy access data
    df.columns=["docs", "year", "topic1", "topic2", "topic3", "topic4", "topic5", "topic6", "topic7", "topic8", "topic9", "topic10"]
    df = df.sum(axis=0)
    print(df)
    n = 2
    ...
    topicweights = []
    for w in weights:
        topicweights.append("Topic " + str(x) + "'s Weight is: " + str(w))
        x = x+1
    ...
    def compute_coherence_values(dictionary, corpus, texts, limit=10, start=2, step=3):
        ...
        for num_topics in range(start, limit, step):
            model = LdaMallet('/Users/nataliecrawford/Downloads/Mallet-202108/bin/mallet', corpus=corpus, num_topics=num_topics,
                              id2word=dictionary, iterations=100)
    ...
    np.savetxt(
        "/Users/nataliecrawford/Documents/Research/topicmodeling/saved.txt",
        np.matrix(d), delimiter=',')
    ...
    model1 = LdaMallet('/Users/nataliecrawford/Downloads/Mallet-202108/bin/mallet',corpus=corpus , num_topics=10, id2word=dictionary, iterations=101)
    #model2 = LdaMallet('/Users/amirkarami/Documents/Research/mallet-2.0.8/bin/mallet',corpus=corpus , num_topics=10, id2word=dictionary, iterations=1000)
    ...
    #written by natalie
    #converts topics array to a string for printing
    n = 1
    topicstr = " "
    topicar = []
    for topic in topics:
        topicstr = topicstr + str(n) + ". " + str(topic)[1:-1] + "\n\n"
        topicar.append(str(n) + ". " + str(topic)[1:-1].replace("'",""))
        n = n+1
    topicstr = topicstr.replace("'", "")
    topicstr = "The topics are \n" + topicstr
    ...
    coherencestr = "The coherence is " +str(coherence) + "\n\n"
    perplexitystr = "The perplexity is "+str(perplexity) + "\n\n"

    #return array for the data to send to render template
    coh = [coherencestr, perplexitystr, "The topics are: "] + topicar + ["The weights per topic are: "] + topicweights
